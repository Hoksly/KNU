...
float medianSingle(vector<int> arr, int beg, int n)
{
    if (n == 0)
        return -1;
    if ((n - beg) % 2 == 0)
...
float findMedianUtil(vector<int> A, int begA, int N, vector<int> B, int begB, int M)
{
    ...
    /* if A[idxA] <= B[idxB], then median must exist in
       A[idxA....] and B[....idxB] */
    if (A[idxA] <= B[idxB])
        return findMedianUtil(A, begA + idxA, N / 2 + 1, B, begB, M - idxA);

    /* if A[idxA] > B[idxB], then median must exist in
       A[...idxA] and B[idxB....] */
    return findMedianUtil(A, begA, N / 2 + 1, B, begB + idxA, M - idxA);
}

// A wrapper function around findMedianUtil(). This function
// makes sure that smaller array is passed as first argument
// to findMedianUtil
float findMedian(vector<int> A, int N, vector<int> B, int M)
{
    if (N > M)
        return findMedianUtil(B, 0, M, A, 0, N);

    return findMedianUtil(A, 0, N, B, 0, M);
}
